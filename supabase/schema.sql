...
create table if not exists public.queue (
  ...
  code text not null unique check (code ~ '^(NC|AD|AG|D|G)-[0-9]+$'),
...
create index if not exists queue_private_patient_name_idx on public.queue_private (lower(patient_name));
